...
python3 -m dataset.csv_builder \
    ...
    --output=dataset/convusersec/twconvrsu_csv_v2i_20k/train.csv \
    --text-field=2 \
...
python3 -m dataset.csv_builder \
    --dataset-root=dataset/convusersec/dialogs \
    --profiles-path=dataset/convusersec/timelines.csv \
    --output=dataset/convusersec/twconvrsu_csv_v2i_20k/valid.csv \
    --text-field=2 \
    --min-context-length=2 \
    --tokenize-tweets \
    -e=5000 \
...
python3 -m dataset.csv_builder \
    --dataset-root=dataset/convusersec/dialogs \
    --profiles-path=dataset/convusersec/timelines.csv \
    --output=dataset/convusersec/twconvrsu_csv_v2i_20k/tests.csv \
    --text-field=2 \
    --min-context-length=2 \
    --tokenize-tweets \
    -e=5000 \
...
python3 -m dataset.tfrecords_builder \
    --input_dir=dataset/convusersec/twconvrsu_csv_v2i_20k \
    --output_dir=dataset/convusersec/twconvrsu_tf_v2i_20k \
    --max_sentence_len=1400

python3 -m dataset.embeddings_builder \
    dataset/convusersec/twconvrsu_tf_v2i_20k/vocabulary.txt \
