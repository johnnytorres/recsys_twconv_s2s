...
python3 -m datasets.csv_builder \
    ...
    --output=datasets/convusersec/twconvrsu_csv_v2i_60k/train.csv \
    --text-field=2 \
...
python3 -m datasets.csv_builder \
    --datasets-root=datasets/convusersec/dialogs \
    --profiles-path=datasets/convusersec/timelines.csv \
    --output=datasets/convusersec/twconvrsu_csv_v2i_60k/valid.csv \
    --text-field=2 \
    --min-context-length=2 \
    --tokenize-tweets \
    -e=5000 \
...
python3 -m datasets.csv_builder \
    --datasets-root=datasets/convusersec/dialogs \
    --profiles-path=datasets/convusersec/timelines.csv \
    --output=datasets/convusersec/twconvrsu_csv_v2i_60k/tests.csv \
    --text-field=2 \
    --min-context-length=2 \
    --tokenize-tweets \
    -e=5000 \
...
python3 -m datasets.tfrecords_builder \
    --input_dir=datasets/convusersec/twconvrsu_csv_v2i_60k \
    --output_dir=datasets/convusersec/twconvrsu_tf_v2i_60k \
    --max_sentence_len=1400

python3 -m datasets.embeddings_builder \
    datasets/convusersec/twconvrsu_tf_v2i_60k/vocabulary.txt \
