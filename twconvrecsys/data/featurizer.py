...
def create_feature_columns(HYPER_PARAMS):
    ...
    The tensorFlow feature_column objects are created based on the datasets types of the features
    defined in the metadata.py module.

    The feature_column(s) are created based on the input features,
    and the constructed features (process_features method in input.py), during reading datasets files.
    ...
    numeric_feature_names = set(metadata.get_input_numeric_features())

    # create t.feature_column.numeric_column columns without scaling
    if feature_stats is None:
        numeric_columns = {feature_name: tf.feature_column.numeric_column(feature_name, normalizer_fn=None, shape=(1,size), dtype=type )
                           for feature_name, size, type in numeric_feature_names}
    ...
    categorical_columns_with_identity = \
        {item[0]: tf.feature_column.categorical_column_with_identity(item[0], item[1])
         for item in list(categorical_feature_names_with_identity.items())}
    ...
    categorical_columns_with_vocabulary = \
        {item[0]: tf.feature_column.categorical_column_with_vocabulary_list(item[0], item[1])
         for item in list(metadata.INPUT_CATEGORICAL_FEATURE_NAMES_WITH_VOCABULARY.items())}
    ...
    categorical_columns_with_hash_bucket = \
        {item[0]: tf.feature_column.categorical_column_with_hash_bucket(item[0], item[1])
         for item in list(metadata.INPUT_CATEGORICAL_FEATURE_NAMES_WITH_HASH_BUCKET.items())}
...
def get_deep_and_wide_columns(feature_columns, use_indicators=True, use_wide_columns=True):
    ...
    """
    dense_columns = list(
        [column for column in feature_columns if isinstance(column, feature_column._NumericColumn) |
                              isinstance(column, feature_column._EmbeddingColumn)]
    )

    categorical_columns = list(
        [column for column in feature_columns if isinstance(column, feature_column._VocabularyListCategoricalColumn) |
                              isinstance(column, feature_column._IdentityCategoricalColumn) |
                              isinstance(column, feature_column._BucketizedColumn)]
    )

    sparse_columns = list(
        [column for column in feature_columns if isinstance(column, feature_column._HashedCategoricalColumn) |
    ...
    if use_indicators:
        indicator_columns = list(
            [tf.feature_column.indicator_column(column) for column in categorical_columns]
        )
